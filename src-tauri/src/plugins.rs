...
pub(crate) fn plugin_tool_label(runtime: &StorageRuntimeState, kind: &str) -> Option<String> {
    let plugin_id = sanitize_plugin_id(kind.strip_prefix("plugin:")?);
    ...
    let installed_root = plugins_dir(runtime).join(&plugin_id);
    if let Some(registration) = normalize_plugin_manifest(&installed_root) {
        return Some(registration.label);
    }
    ...
    candidate_bases
        ...
        .flatten()
        .map(|base| base.join(&plugin_id))
        .find_map(|root| normalize_plugin_manifest(&root).map(|registration| registration.label))
}
