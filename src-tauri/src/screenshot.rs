...
fn notify_screenshot_overlay_session_end(app: &AppHandle) {
    log_flow("screenshot", "session-end emit");
    let _ = app.emit("screenshot:session-end", ());
}

fn schedule_close_screenshot_overlay(app: AppHandle, label: String) {
    notify_screenshot_overlay_session_end(&app);
    clear_screenshot_color_frames(&app);

    let region_state = app.state::<crate::screen_recorder::RegionRecordingState>();
    if crate::screen_recorder::is_screenshot_overlay_reserved_for_recording(&region_state, &label) {
        log_flow(
            "screenshot",
            format!("keep idle overlay label={label} region_recording_active"),
        );
        return;
    }

    log_flow("screenshot", format!("keep idle overlay label={label}"));
}
