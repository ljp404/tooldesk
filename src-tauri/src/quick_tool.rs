...
fn resolve_quick_tool_window_title(runtime: &StorageRuntimeState, kind: &str) -> String {
    plugins::plugin_tool_label(runtime, kind)
        .unwrap_or_else(|| quick_tool_window_title(kind).to_string())
}
...
fn open_quick_tool_window(
    ...
    if !options.force_new {
        if let Some(window) = app.get_webview_window(&label) {
            ...
            if options.compact {
                let size = quick_tool_window_size(&kind, true);
    ...
    .title(resolve_quick_tool_window_title(runtime, &kind))
...
pub(crate) fn warm_taskbar_calendar_popup(app: AppHandle) {
    thread::spawn(move || {
        ...
        let _ = app.run_on_main_thread(move || {
            ...
            if let Some(window) =
                app_for_main.get_webview_window("quick-compact-plugin:tooldesk-calendar")
            {
                log_window_state("calendar", "warm skipped existing", &window);
                return;
            }

            if !plugins::is_plugin_installed(&runtime, "tooldesk-calendar") {
                log_flow("calendar", "warm skipped plugin_uninstalled");
                return;
            }

            let size = quick_tool_window_size(TASKBAR_CALENDAR_TOOL_KEY, true);
            let mut builder = WebviewWindowBuilder::new(
                &app_for_main,
                "quick-compact-plugin:tooldesk-calendar",
                WebviewUrl::App(build_quick_tool_url(TASKBAR_CALENDAR_TOOL_KEY, true).into()),
            )
            .always_on_top(true)
            .title(resolve_quick_tool_window_title(
                &runtime,
                TASKBAR_CALENDAR_TOOL_KEY,
            ))
            .inner_size(size.width, size.height)
            .min_inner_size(size.min_width, size.min_height)
            .resizable(false)
            ...
            if let Some((x, y)) = anchored_position(&app_for_main, &size, None) {
                builder = builder.position(x, y);
            }
            ...
            match builder.build() {
                Ok(window) => {
                    crate::window::hide_window_from_taskbar(&window);
                    let app_handle = app_for_main.clone();
                    let window_label = window.label().to_string();
                    window.on_window_event(move |event| {
                        if matches!(event, WindowEvent::Focused(false)) {
                            if screenshot::is_screenshot_overlay_active(&app_handle) {
                    ...
                    log_window_state("calendar", "warm built", &window);
                    log_quick_windows_snapshot(&app_for_main, "after calendar warm");
                }
                Err(error) => {
                    log_flow("calendar", format!("warm failed error={error}"));
                    log_quick_windows_snapshot(&app_for_main, "after calendar warm failed");
                }
            }
        });
    });
