...
static HOTZONE_BOUNDS_READY: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "windows")]
static HOTZONE_LEFT: AtomicI32 = AtomicI32::new(0);
#[cfg(target_os = "windows")]
static HOTZONE_TOP: AtomicI32 = AtomicI32::new(0);
#[cfg(target_os = "windows")]
static HOTZONE_RIGHT: AtomicI32 = AtomicI32::new(0);
#[cfg(target_os = "windows")]
static HOTZONE_BOTTOM: AtomicI32 = AtomicI32::new(0);
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
fn cache_hotzone_bounds(bounds: HotzoneBounds) {
    HOTZONE_LEFT.store(bounds.x.round() as i32, Ordering::Relaxed);
    HOTZONE_TOP.store(bounds.y.round() as i32, Ordering::Relaxed);
    HOTZONE_RIGHT.store((bounds.x + bounds.width).round() as i32, Ordering::Relaxed);
    HOTZONE_BOTTOM.store((bounds.y + bounds.height).round() as i32, Ordering::Relaxed);
    HOTZONE_BOUNDS_READY.store(true, Ordering::Release);
}

#[cfg(target_os = "windows")]
fn point_in_cached_hotzone(x: i32, y: i32) -> bool {
    if !HOTZONE_BOUNDS_READY.load(Ordering::Acquire) {
        return false;
    }

    x >= HOTZONE_LEFT.load(Ordering::Relaxed)
        && x <= HOTZONE_RIGHT.load(Ordering::Relaxed)
        && y >= HOTZONE_TOP.load(Ordering::Relaxed)
        && y <= HOTZONE_BOTTOM.load(Ordering::Relaxed)
}
...
fn apply_hotzone_bounds(app: &AppHandle) -> bool {
    ...
    let Some(bounds) = resolve_hotzone_bounds(app) else {
        let _ = window.close();
        #[cfg(target_os = "windows")]
        HOTZONE_BOUNDS_READY.store(false, Ordering::Release);
        return false;
    };
    #[cfg(target_os = "windows")]
    cache_hotzone_bounds(bounds);
    ...
    let _ = window.show();
    crate::window::hide_window_from_taskbar(&window);
    true
}
...
#[cfg(target_os = "windows")]
fn should_open_from_cached_native_click(x: i32, y: i32) -> bool {
    if !point_in_cached_hotzone(x, y) {
        return false;
    }
...
#[cfg(target_os = "windows")]
fn schedule_hide_calendar_popup_if_outside(app: AppHandle, x: i32, y: i32) {
    let app_for_main = app.clone();
    let _ = app.run_on_main_thread(move || {
        hide_calendar_popup_if_outside(&app_for_main, x, y);
    });
}

#[cfg(target_os = "windows")]
fn schedule_open_taskbar_calendar_popup(app: AppHandle, x: i32, y: i32) {
    let app_for_main = app.clone();
    let _ = app.run_on_main_thread(move || {
        log_flow("calendar", format!("native click x={x} y={y}"));
        let runtime = app_for_main.state::<StorageRuntimeState>();
        let shortcut_content = app_for_main.state::<ShortcutContentState>();
        let _ = open_taskbar_calendar_popup_inner(&app_for_main, &runtime, &shortcut_content);
    });
}
            ...
            if code >= 0 && (w_param as u32 == WM_LBUTTONDOWN || w_param as u32 == WM_LBUTTONUP) {
                ...
                if let Some(app) = app {
                    if w_param as u32 == WM_LBUTTONDOWN {
                        schedule_hide_calendar_popup_if_outside(app.clone(), info.pt.x, info.pt.y);
                    }

                    if !point_in_cached_hotzone(info.pt.x, info.pt.y) {
                        return CallNextHookEx(0 as HHOOK, code, w_param, l_param);
                    }

                    if w_param as u32 == WM_LBUTTONDOWN
                        && should_open_from_cached_native_click(info.pt.x, info.pt.y)
                    {
                        schedule_open_taskbar_calendar_popup(app, info.pt.x, info.pt.y);
                    }
