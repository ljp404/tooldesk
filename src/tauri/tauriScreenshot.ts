...
import { captureScreenshotImageToSuperClipboard } from './tauriSuperClipboard';
...
export function onScreenshotCaptured(callback: (result: ScreenshotCaptureResult | null) => void) {
  ...
  ensureScreenshotCapturedListener();
...
function ensureScreenshotCapturedListener() {
  ...
  void listen<ScreenshotCaptureResult | null>('screenshot:captured', (event) => {
    ...
    if (event.payload?.dataUrl) {
      captureScreenshotImageToSuperClipboard(event.payload.dataUrl);
    }
...
export function initScreenshotCapturedListener() {
  ensureScreenshotCapturedListener();
}
